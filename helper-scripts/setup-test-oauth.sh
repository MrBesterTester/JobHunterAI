...
# Save Gmail tokens immediately to .env.test
...
if [ -f "$PROJECT_ROOT/.env.test" ]; then
    ...
    # Create new file with Gmail tokens only
...
# Save Microsoft tokens immediately to .env.test
...
if [ -f "$PROJECT_ROOT/.env.test" ]; then
    ...
    # This shouldn't happen (Gmail should have created it), but handle it
    log_error "Warning: .env.test not found, creating with Microsoft tokens only"
    cat > "$PROJECT_ROOT/.env.test" << ENVEOF
# JobHunter Test Environment OAuth Configuration
# Auto-generated by setup-test-oauth.sh on $(date '+%Y-%m-%d %H:%M:%S %Z')

# Gmail OAuth Configuration (not configured)
...
# ============================================================================
# Update .env.test with complete documentation
# ============================================================================

log_section "Finalizing .env.test"

# Now overwrite with complete documented version
cat > "$PROJECT_ROOT/.env.test" << ENVEOF
# JobHunter Test Environment OAuth Configuration
# Auto-generated by setup-test-oauth.sh on $(date '+%Y-%m-%d %H:%M:%S %Z')
#
